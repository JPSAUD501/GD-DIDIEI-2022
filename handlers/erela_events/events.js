          ...
          collector.on('collect', async i => {
            ...
            if (i.customId == `10`) {
                ...
                embeds: [new MessageEmbed()
                    ...
                    .setTitle(`Estou procurando a letra dessa música pra você: ${emoji.msg.search} \`${title}\``.substr(0, 256))
                ]})
                ...
                title = title.replace(/\]/gi, ' ');
                title = title.replace(/#/gi, ' ');
                title = title.replace(/ music /gi, ' ');
                title = title.replace(/ official /gi, ' ');
                title = title.replace(/ video /gi, ' ');
                title = title.replace(/ oficial /gi, ' ');
                title = title.replace(/ videoclipe /gi, ' ');
                title = title.replace(/ lyrics /gi, ' ');
                title = title.replace(/ musical /gi, ' ');
                title = title.replace(/ clipe /gi, ' ');
                title = title.replace(/ m\/v /gi, ' ');
                title = title.replace(/ track /gi, ' ');
                title = title.replace(/ ft /gi, ' ');
                title = title.replace(/ ft\. /gi, ' ');
                title = title.replace(/  /gi, ' ');
                title = title.replace(/  /gi, ' ');
                ...
                return swap_pages(client, i, lyrics, `Letras de: ${emoji.msg.lyrics} \`${titledisplay}\``.substr(0, 256));
              //
            }
          });
        }
