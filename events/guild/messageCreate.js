 ...
 */
const channellist = process.env['channellist'] //Lista dos canais suportados pelo bot
...
module.exports = async (client, message) => {
      try {
        ...
        if (channellist !== "null"){
          if (!channellist.includes(message.channel.id) && !channellist.includes(message.channel.name)) return;
        }
        ...
        if (!prefixRegex.test(message.content)){
          ...
          if((args.toString().includes("playlist") && args.toString().includes("http") && !args.toString().includes("twitch.tv")) || args.toString().toLowerCase().includes("playlist")){
            var cmd = "playlist";
          }else{
            var cmd = cmdpadrao.toLowerCase();
          }
          //console.log(cmd);
          var [, matchedPrefix] = (prefix+cmd+" "+message.content).match(prefixRegex);
          try {
